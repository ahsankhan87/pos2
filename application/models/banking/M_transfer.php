...
class M_transfer extends CI_Model{
    ...
    public function get_transfer($id = FALSE, $limit = 1000000, $offset = 0)
    {
        if($id === FALSE)
        {
            if($limit)
            ...
            $query = $this->db->get_where('acc_transfer',$options);
    ...
    //retrieve journal transfer against ledger account.
    public function balanceByAccount($account_code,$fy_start_date,$fy_end_date)
    {
        ...
        // $this->db->join('acc_transfer e','e.id=ei.entry_id');
    ...
    public function get_alltransfer($company_id,$fy_start_date,$fy_end_date)
    {
        ...
        $options = array('ei.company_id'=> $company_id,'g.company_id'=> $company_id);
        
        $this->db->where('ei.date >=', $fy_start_date);
        $this->db->where('ei.date <=', $fy_end_date);

        $query = $this->db->get_where('acc_transfer ei',$options);
        $data = $query->result_array();
        return $data;
    }
    
    public function get_Todaytransfer($company_id,$date)
    {
        $this->db->group_by('ei.id');
        $this->db->select('g.title,g.title_ur, ei.*,e.entry_no');
        $this->db->join('acc_transfer e','ei.entry_id=e.id');
        $this->db->join('acc_groups g','ei.account_code=g.account_code');
        //$this->db->group_by('l.id');
    ...
    public function get_transferByNo($company_id,$fy_start_date,$fy_end_date,$entry_no)
    {
    ...
    function addtransfer($dr_ledger,$cr_ledger,$dr_amount,$cr_amount,$narration='',$invoice_no='',
    ...
    function edittransfer()
    {
       ...
       $this->db->update('acc_transfer', $data, array('id'=>$this->input->post('id')));
    ...
    function check_entry_no($entry_no)
     {
        ...
        $query = $this->db->get('acc_transfer'); 
    ...
    public function get_transferByEntryNo($entry_no)
    {
    ...
    public function get_transferByDate($date)
    {
