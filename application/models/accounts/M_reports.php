...
class M_reports extends CI_Model
{
    ...
    function get_net_income($fy_start_date, $fy_end_date)
    {
        $query_string =
    ...
    function year_report($company_id, $month, $year, $account_code)
    {
        if ($this->db->dbdriver === 'sqlite3') {
            ...
            $this->db->where(array(
                'ei.company_id' => $company_id, 'g.company_id' => $company_id, 'strftime("%m",ei.date)' => $month,
            ...
            $this->db->where(array(
                'ei.company_id' => $company_id, 'g.company_id' => $company_id, 'MONTH(ei.date)' => $month,
    ...
    function get_amount_by_month($company_id, $month, $account_code)
    {
        //FOR WEB APP ONLY
        $this->db->select('MONTH(ei.date) as month,SUM(debit-credit) as balance');
        $this->db->group_by('MONTH(ei.date)');
        $this->db->where(array(
            'ei.company_id' => $company_id, 'MONTH(ei.date)' => $month,
            'ei.account_code' => $account_code
        ));
        // $this->db->join('acc_entry_items ei', 'ei.account_code=g.account_code');
