...
//        $year = date('y', $unixtime );
    ...
    public function category_wise_sales($from_date=null,$to_date=null,$company_id,$emp_id=null,$register_mode='sale',$sale_type='cash'){
       ...
       $this->db->select('SUM(A.total_amount) as amount,A.sale_date,A.employee_id, B.item_id,
       B.category as category,A.invoice_no,SUM(B.quantity_sold) as qty');
       $this->db->join('sale_report AS B','A.invoice_no = B.invoice_no');
       //$this->db->join('pos_sizes AS C','B.size_id = C.size_id');
       $this->db->where('A.company_id',$company_id);
