...
class M_customers extends CI_Model
{
    ...
    public function get_customers_suppliers()
    {
        $query = $this->db->query('SELECT id,first_name,last_name,store_name,"customer" FROM pos_customers as cust UNION SELECT id, name, address, contact_no,"vendor" FROM pos_supplier as sup');
        //$this->db->select('id,first_name,last_name,store_name,address,email,city,mobile_no,op_balance_dr,op_balance_cr');
        $this->db->where('cust.status', 'active');
        $this->db->where('cust.company_id', $_SESSION['company_id']);

        $this->db->order_by('cust.id', 'desc');
        //$query = $this->db->get_where('pos_customers', $options);
        $data = $query->result_array();
        return $data;
    }
    ...
    {
        $this->db->select('SUM(debit) as dr_balance, SUM(credit) as cr_balance, SUM(debit-credit) as total_balance')->from('pos_customer_payments sp')->where('sp.customer_id', $customer_id);
        $this->db->where('sp.company_id', $_SESSION['company_id']);
        $this->db->where('date >=', $fy_start_date);
        $this->db->where('date <=', $fy_end_date);
        $query = $this->db->get();
    ...
    function addCustomerPaymentEntry(
        $account_code,
        $dueTo_acc_code,
        $dr = 0,
        $cr = 0,
    ...
    {
        $query = $this->db->get_where('pos_customer_payments', array(
            'entry_id' => $entry_id,
            'company_id' => $_SESSION['company_id']
        ));
