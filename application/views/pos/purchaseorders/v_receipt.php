...
<button class="hidden-print"><a href="<?php echo site_url('trans/C_purchaseOrders/allPurchaseorders') ?>">Purchase Orders</a></button <!-- BEGIN PAGE CONTENT-->
<div class="invoice">
    <div class="row invoice-logo">
        ...
        <div class="col-sm-6 col-xs-6">
            <p>
                ...
                #<?php echo $invoice_no; ?> / <?php echo date('d, M Y', strtotime(@$purchaseorders_items[0]['receiving_date'])); ?>
                <span>User: <?php echo @$this->M_users->get_activeUsers(@$purchaseorders_items[0]['user_id'])[0]['name']; ?><br />
                    <?php echo lang('name'); ?>:&nbsp;<?php echo @$this->M_suppliers->get_supplierName(@$purchaseorders_items[0]['supplier_id']); ?>
                </span>
            </p>
        </div>
    </div>
        ...
        <div class="col-xs-12">
            <table class="table table-striped table-bordered table-hover table-condensed">
                ...
                <tbody>
                    ...
                    $discount_total = 0;
                    $total_tax_amount = 0;
                    if ($purchaseorders_items) {
                        foreach ($purchaseorders_items as $key => $list) {
                            if ($list['service']) {
                            ...
                            if (@$_SESSION['multi_currency'] == 1) {
                                $currency = $this->M_currencies->get_currencies($purchaseorders_items[0]['currency_id']);
    ...
    <div class="row">
        <div class="col-xs-8">
            <?php if (@$purchaseorders_items[0]['description']) { ?>
                <br />
                <div class="well">
                    <?php echo @$purchaseorders_items[0]['description']; ?>
                </div>
            <?php } ?>
        </div>
        <div class="col-xs-4 invoice-block">
            <ul class="list-unstyled amounts">
                ...
                <li>
                    <strong>Discount:</strong> <?php echo $discount_total = round(@$purchaseorders_items[0]['discount_value'], 2); ?>
                </li>
                <li>
