...
class C_suppliers extends MY_Controller
{
    ...
    function BS_form_pdf_report($supplier_id)
    {
        // var_dump($sales_items);
        $cur_balance = $this->M_suppliers->get_supplier_total_balance($supplier_id, FY_START_DATE, FY_END_DATE);
        $SupplierName = $this->M_suppliers->get_SupplierName($supplier_id);
        $company_id = $_SESSION['company_id'];
        $Company = $this->M_companies->get_companies($company_id);
        ...
        $pdf->AddPage();
        $pdf->SetTitle(lang('bs_title'));

        //Display form text
        $pdf->SetY(15);
        // $pdf->SetX(40);
        $pdf->SetFont('DejaVu', '', 14);
        $pdf->Cell(180, 10, lang('bs_title'), 0, 10, "C");

        $pdf->Ln(5);
        $pdf->SetFont('DejaVuBold', 'B', 12);
        $pdf->Cell(180, 6, lang("dear") . ' ' . $SupplierName . ",");

        $body_text_1 = lang('bs_para_1');
        $pdf->Ln(10);
        $pdf->SetFont('DejaVu', '', 12);
        $pdf->MultiCell(180, 6, $body_text_1);

        $body_text_2 = lang('bs_para_2');
        $pdf->Ln(5);
        $pdf->SetFont('DejaVu', '', 12);
