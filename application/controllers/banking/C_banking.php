...
class C_banking extends MY_Controller{
    ...
    function bank_transactions($bank_id)
    {
        $data = array('langs' => $this->session->userdata('lang'));
    ...
    public function save_bank_transaction()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->M_banking->addBankPaymentEntry($deposit_to_acc_code,$acc_code_2,$total_amount,0,$bank_id,$narration,
            $new_invoice_no,$sale_date,$entry_id);
            //for logging
